...
export const getVehicles = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM t_vehiculo ORDER BY veh.vehid ASC ');
    res.json(rows);
  } catch (error) {
...
export const getVehicle = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT * FROM t_vehiculo ORDER BY veh.vehid ASC  WHERE veh.vehid = ? ', [
      vehid,
    ]);
